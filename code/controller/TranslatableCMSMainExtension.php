...
class TranslatableCMSMainExtension extends Extension {
	...
	function updateEditForm(&$form) {
        ...
        $fields = $form->Fields();
        $siteConfig = Translatable::get_site_config();

		if($formName == 'RootForm' && SiteConfig::has_extension("Translatable")) {
			$fields->push(new HiddenField('Locale','', $siteConfig->Locale));
		}

        /** @var FieldList $fields */
        $fields->insertBefore(
            new LabelField('LocaleIndicator', "<h3>Viewing locale $siteConfig->Locale</h3>"),
            'Title'
        );
